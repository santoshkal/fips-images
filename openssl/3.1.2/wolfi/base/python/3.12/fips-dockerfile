...
RUN /opt/python-fips/bin/python3.12 -m ensurepip \
    && /opt/python-fips/bin/python3.12 -m pip install --no-cache-dir wheel \
    && /opt/python-fips/bin/python3.12 -m pip install --no-cache-dir --upgrade pip setuptools \
...
RUN find /opt/python-fips -depth \
    ...
    -exec rm -rf {} + \
    && strip --strip-unneeded /usr/local/ssl/bin/openssl /opt/python-fips/bin/python3.12 2>/dev/null || true
