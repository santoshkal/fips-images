...
###############################
# Stage 1: Build OpenSSL 3.1.2 with FIPS on Alpine
###############################
...
# Build OpenSSL 3.1.2 with FIPS provider
...
###############################
# Stage 2: Build Python 3.14.0 with FIPS OpenSSL
###############################
...
RUN /opt/python-fips/bin/python3.14 -m ensurepip \
 && /opt/python-fips/bin/python3.14 -m pip install --no-cache-dir --upgrade pip setuptools wheel \
 && CRYPTOGRAPHY_DONT_BUILD_RUST=1 \
    CFLAGS="-I/usr/local/ssl/include" \
    LDFLAGS="-L/usr/local/ssl/lib" \
    /opt/python-fips/bin/python3.14 -m pip install --no-cache-dir cryptography \
 && ln -sf /opt/python-fips/bin/pip3.14 /opt/python-fips/bin/pip

# Strip binaries for smaller size
RUN strip --strip-unneeded /usr/local/ssl/bin/openssl /opt/python-fips/bin/python3.14 || true
...
RUN ln -sf /usr/local/ssl/bin/openssl /usr/bin/openssl \
 ...
 && ln -sf /opt/python-fips/bin/python3.14 /usr/bin/python3 \
 && ln -sf /opt/python-fips/bin/python3.14 /usr/local/bin/python3 \
 && ln -sf /opt/python-fips/bin/python3.14 /usr/bin/python \
 && ln -sf /opt/python-fips/bin/pip3.14 /usr/bin/pip \
 && ln -sf /opt/python-fips/bin/pip3.14 /usr/local/bin/pip
