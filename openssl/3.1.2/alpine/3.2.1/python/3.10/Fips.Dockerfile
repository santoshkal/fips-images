...
# ------------------------------------------------------------------------------
# Stage 2: Build Stage - Compile OpenSSL FIPS
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# Build OpenSSL 3.1.2 with FIPS Provider (Certificate #4282)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# Build Python 3.10.13 with FIPS-enabled OpenSSL
# ------------------------------------------------------------------------------
...
RUN echo "==> Installing Python packages with FIPS support..." \
  ...
  # Install ensurepip
  && /opt/python-fips/bin/python3.10 -m ensurepip \
  # Upgrade pip and setuptools
  && /opt/python-fips/bin/python3.10 -m pip install --no-cache-dir \
  ...
  /opt/python-fips/bin/python3.10 -m pip install --no-cache-dir cryptography
...
RUN echo "==> Optimizing Python installation..." \
  ...
  && ln -sf /opt/python-fips/bin/python3.10 /opt/python-fips/bin/python3 \
  && ln -sf /opt/python-fips/bin/python3.10 /opt/python-fips/bin/python \
  && ln -sf /opt/python-fips/bin/pip3.10 /opt/python-fips/bin/pip3 \
  && ln -sf /opt/python-fips/bin/pip3.10 /opt/python-fips/bin/pip \
  ...
  && strip --strip-unneeded /opt/python-fips/bin/python3.10 2>/dev/null || true \
...
RUN ln -sf /usr/local/ssl/bin/openssl /usr/bin/openssl \
  ...
  && ln -sf /opt/python-fips/bin/python3.10 /usr/bin/python3 \
  && ln -sf /opt/python-fips/bin/python3.10 /usr/local/bin/python3 \
  && ln -sf /opt/python-fips/bin/python3.10 /usr/bin/python \
  && ln -sf /opt/python-fips/bin/python3.10 /usr/local/bin/python \
