...
' Modules/_hashopenssl.c \
    ...
    && ldconfig -v \
    && ln -s -f /opt/python-fips/bin/python3.13 /usr/bin/python3 \
    && ln -s -f /opt/python-fips/bin/python3.13 /usr/local/bin/python3 \
    && ln -s -f /opt/python-fips/bin/python3.13 /usr/bin/python
...
RUN /opt/python-fips/bin/python3 -m ensurepip \
    ...
    && CRYPTOGRAPHY_DONT_BUILD_RUST=1 \
       ...
       /opt/python-fips/bin/python3 -m pip install --no-cache-dir \
         cryptography==46.0.3 \
    && ln -s -f /opt/python-fips/bin/pip3.13 /opt/python-fips/bin/pip \
    && rm -rf Python-${PYTHON_VERSION} \
    && find /opt/python-fips -depth \
       \( -name 'test' -o -name 'tests' -o -name '*.pyc' -o -name '*.pyo' -o -name 'idlelib' -o -name 'tkinter' \) \
       -exec rm -rf {} + \
    && strip --strip-unneeded /usr/local/ssl/bin/openssl /opt/python-fips/bin/python3.13
